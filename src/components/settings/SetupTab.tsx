...
import { fetchClassTeacherAssignments, setClassTeacher } from '../../lib/attendanceApi'
import { fetchTeachersForAssignment } from '../../lib/staff/staffApi'
import {
...
export function SetupTab() {
  ...
  const { data: classTeacherAssignments = [] } = useQuery({
    ...
    queryFn: fetchClassTeacherAssignments,
  })
  const { data: teachers = [] } = useQuery({
    queryKey: ['teachers-for-assignment'],
    queryFn: fetchTeachersForAssignment,
  })
  ...
  const setClassTeacherMutation = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['class-teacher-assignments'] }),
  })
  ...
  async function handleLogoChange(e: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      void logError(err, { type: 'school_logo_upload' })
      setLogoError(getUserFriendlyError(err, 'The school logo could not be uploaded. Please try again.'))
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-xl border border-gray-200 bg-white p-6">
        <h3 className="text-base font-semibold text-gray-900">Class Teachers</h3>
        <p className="mt-1 text-sm text-gray-500">
          The class teacher is responsible for that class's daily attendance.
        </p>
        <div className="mt-3 space-y-2">
          {classTeacherAssignments.map((a) => (
            <div key={a.class_id} className="flex items-center justify-between gap-3">
              <span className="text-sm text-gray-700">{a.class_name}</span>
              <select
                value={a.teacher_id ?? ''}
                onChange={(e) =>
                  setClassTeacherMutation.mutate({
                    classId: a.class_id,
                    teacherId: e.target.value === '' ? null : e.target.value,
                  })
                }
